...
\textbf{Datasets.} The resources behind these three families arrived in a clear order. Anthropic's model-written evaluations (2022) produced 154 test sets, among them multiple-choice items where a biography states the user's politics before the model is asked for its own view, so agreement could only be scored as picking the matching option~\citep{DiscoveringLanguageModelBehaviors_Perez}. SycophancyEval (2023) moved to free text, putting five production assistants through four open tasks, among them feedback on a user's work and copying a mistake the user made~\citep{TowardsUnderstandingSycophancy_Sharma}. Both still need a stated belief and a checkable answer. ELEPHANT (2025) needs neither, scoring 3,027 open-ended advice queries, 2,000 Reddit posts where readers judged the writer at fault, 3,777 assumption-laden posts from a Reddit advice forum, and 1,591 moral conflicts retold from each side~\citep{ElephantMeasuringSocial_Cheng}. ELEPHANT reads one reply at a time, so SYCON-Bench (2025) stretched the setting over five turns, building 500 dialogues from 100 debate topics, 200 stereotype prompts rewritten from StereoSet, and 200 questions carrying a false premise~\citep{MeasuringSycophancyMultiturn_Hong}. Neither set shows which social cue does the work, and GaslightBench (2025) answers with 80 easily checked facts, each one falsified and wrapped in ten families of modifier such as flattery or a made-up citation, giving 24,240 single-turn prompts and 720 four-turn dialogues~\citep{GaslightBenchQuantifyingLLM_Yao}. Three more 2025 sets push on stakes, realism, and mechanism. Fifty brand and generic drug pairs ask the model to say why one drug is safer than the other, a request that is false by construction~\citep{WhenHelpfulnessBackfiresLLMs_Chen}. A context corpus swaps invented users for two weeks of real chat history from 38 people~\citep{InteractionContextOftenIncreases_Jain}. Nine synthetic arithmetic and factual sets vary the user's claim and the presence of praise one at a time, so agreement and flattery can be told apart inside the model~\citep{SycophancyIsNotOne_Vennemeyer}. Beacon (2025) drops conversation altogether, offering 420 hand-written prompts each paired with one principled reply and one flattering reply~\citep{BeaconSingleTurnDiagnosis_Pandey}. The 2026 sets then isolate wording, language, and domain in turn. Ask Don't Tell turns 40 debatable yes-or-no questions into eleven matched non-question versions apiece, varying how certain and how personal the user sounds~\citep{AskDontTellReducing_Dubois}. llm-bias-bench covers 38 contested topics in Brazilian Portuguese and compares asking for an opinion outright with never asking and letting a debate draw one out~\citep{MeasuringOpinionBiasAnd_Nogueira}. MedPRESS closes the line with 600 medically grounded five-turn dialogues across three scenario families, where patient pressure escalates from personal experience to open accusation~\citep{MedPRESSPatientPressureInduced_Joy}.
