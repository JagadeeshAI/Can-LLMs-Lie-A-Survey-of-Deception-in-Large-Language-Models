...
\midrule
\citet{DoAsWeDo_Weng} & peer majority, Trust/Doubt priming & 9 models & Doubt-protocol accuracy drop up to 38.6\% \\
\citet{NotJustRLHFWhy_TODO} & multi-agent peer disagreement & 4 families (base + instruct) & yield 44--98\%; base $\geq$ instruct in 10/12 cells \\
\citet{MostLLMConformityNeeds_Hu} & bare repeated assertion vs.\ attributed source & 6 open-weight models & 66.5\% floor with no speaker; +12.9pp for expert-panel framing \\
\citet{ItsNotAlwaysSycophancy_TODO} & distractor + authority framing & 7 models & conformity 29.0\% (certain) vs.\ 39.6\% (uncertain) \\
\citet{LLMsCantHandlePeer_Song} & multi-agent support/opposition, rapport level & wide range & avg.\ $\leq$32B models $-$5.65\%, $>$32B $-$3.64\% \\
\citet{HumanLikeSocialCompliance_Zhang} & single-authority + 3-agent consensus & 15 models & sycophancy--conformity correlation $R^2=0.31$ \\
\bottomrule
\end{tabular}
\end{table}
